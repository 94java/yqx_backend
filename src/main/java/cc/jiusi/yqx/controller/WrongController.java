...
public class WrongController {
     ...
     * @return List<Wrong> 实例对象列表
     */
    @GetMapping("/currentWrongBanks")
    @ApiOperation("获取当前用户错题形成的题库列表")
    public BaseResponse<List<Wrong>> currentWrongBanks() {
        return ResultUtils.success(wrongService.currentWrongBanks());
    }
     ...
     * @return 新增结果
     */
    @PostMapping("/changeWrongCount")
    @ApiOperation("修改错题次数")
    public BaseResponse<Integer> changeWrongCount(@RequestBody Wrong wrong) {
        return ResultUtils.success(wrongService.changeWrongCount(wrong));
    }
